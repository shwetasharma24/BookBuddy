...
export default function Book({ book, handleModalOpen, buttons = true, fullsize = false, sizeOfCards = 0, children }) {
    ...
    return (
        <Grid item xs={12} md={fullsize ? 12 : cardSizes[sizeOfCards]}>
            <Paper className={cx("book-wrapper", isMarked ? classes.paperMarked : classes.paper)}>
                <div className="book">
                    <img className="book__img" onClick={handleModalOpen} src={book.image} alt="" />
                    <div className="book__info">
                        ...
                        {!fullsize && (
                            <span onClick={handleProfilePageLink} className="book__addedBy">
                                added by: <span className="book__addedByHover">{book.addedBy}</span>
                            </span>
                        )}
...
const useStyles = makeStyles((theme) => ({
    ...
    paperMarked: {
        ...
        position: "relative",
        color: theme.palette.text.secondary,
        boxShadow: "10px 15px 10px rgba(0, 20, 140, 0.15), 10px 15px 10px rgba(0, 20, 140, 0.15)",
        transition: ".5s",
        [theme.breakpoints.down("md")]: {
